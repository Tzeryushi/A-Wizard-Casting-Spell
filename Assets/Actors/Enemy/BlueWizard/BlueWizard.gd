...
func run_sight_follow_logic() -> void:
	if player_ref:
		...
		if player_seen or player_following:
			if ((player_ref.global_position-global_position).length() < _distance_to_keep+150) and ((player_ref.global_position-global_position).length() > _distance_to_keep-150) and is_shot_obstructed():
